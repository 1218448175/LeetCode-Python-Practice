...
* [0066-加一](./题目目录/0066-plus-one) —— 重点：进位传播模拟，逐位加一统一起始加一与进位
...
- [0142-环形链表 II](./题目目录/0142-linked-list-cycle-ii) —— 重点：Floyd 两阶段判圈，数学证明定位环入口
...
- [0050-Pow(x, n)](./题目目录/0050-powx-n) —— 重点：快速幂（二进制指数拆分），O(log n) 迭代实现
- [0149-直线上最多的点数](./题目目录/0149-max-points-on-a-line) —— 重点：枚举基准点 + 斜率哈希分组统计，O(n²)
- [0007-整数反转](./题目目录/0007-reverse-integer) —— 重点：按位取模翻转，符号分离 + 溢出边界判定
- [0009-回文数](./题目目录/0009-palindrome-number) —— 重点：字符串对称比较，进阶可用反转一半数字实现 O(1) 空间
- [0172-阶乘后的零](./题目目录/0172-factorial-trailing-zeroes) —— 重点：因子 5 计数，尾随零由 2×5 配对决定，统计 5 的幂次
...
- [0005-最长回文子串](./题目目录/0005-longest-palindromic-substring) —— 重点：区间 DP 按子串长度递推，全 True 初始化巧设 base case
- [0070-爬楼梯](./题目目录/0070-climbing-stairs) —— 重点：斐波那契递推 + 双变量滚动迭代，O(1) 空间
- [0188-买卖股票的最佳时机 IV](./题目目录/0188-best-time-to-buy-and-sell-stock-iv) —— 重点：泛化 2k+1 状态机 DP，一维数组原地滚动，k≥n/2 退化为贪心
- [0198-打家劫舍](./题目目录/0198-house-robber) —— 重点：选与不选的 DP 决策，双变量滚动优化到 O(1)
- [0139-单词拆分](./题目目录/0139-word-break) —— 重点：前缀 DP + max_len 剪枝，可变范围扫描
- [0063-不同路径 II](./题目目录/0063-unique-paths-ii) —— 重点：网格 DP + 障碍物处理，一维滚动数组 + 合循环分支
- [0064-最小路径和](./题目目录/0064-minimum-path-sum) —— 重点：二维网格 DP，滚动数组优化至 O(n)，也可原地 O(1)
- [0072-编辑距离](./题目目录/0072-edit-distance) —— 重点：经典 Levenshtein 距离，三操作对应三子问题，min(删/插/换)+1
- [0097-交错字符串](./题目目录/0097-interleaving-string) —— 重点：二维字符串 DP 路径模型，合循环 + 前缀比较处理边界
- [0118-杨辉三角](./题目目录/0118-pascals-triangle) —— 重点：迭代 DP 逐行构造，`ans[-1]` 引用上一行递推
- [0119-杨辉三角 II](./题目目录/0119-pascals-triangle-ii) —— 重点：倒序遍历 + 动态增长滚动数组，O(k) 空间
- [0120-三角形最小路径和](./题目目录/0120-triangle) —— 重点：二维 DP 自顶向下，也可自底向上 O(n) 一维优化
- [0123-买卖股票的最佳时机 III](./题目目录/0123-best-time-to-buy-and-sell-stock-iii) —— 重点：五状态机 DP，最多两笔交易，四变量 O(1) 空间
- [0221-最大正方形](./题目目录/0221-maximal-square) —— 重点：右下角视角 DP，min(左,上,左上)+1，木桶短板原理
- [0300-最长递增子序列](./题目目录/0300-longest-increasing-subsequence) —— 重点：线性 DP，以每个元素为结尾的 LIS 长度递推，O(n²)
- [0322-零钱兑换](./题目目录/0322-coin-change) —— 重点：完全背包二维递推，每种硬币无限使用
